...
// .wrangler/tmp/pages-oT3VVh/functionsWorker-0.031166312274151342.mjs
...
async function onRequestGet6({ request, env }) {
  ...
  const opts = {
    ...
    fromYear: clampYear3(url.searchParams.get("from_year")),
    toYear: clampYear3(url.searchParams.get("to_year")),
  ...
  const tokenRes = await getToken3(env);
  if (!tokenRes.token) {
    return json5({ error: "Spotify auth failed", detail: tokenRes.error }, 502);
  }
...
__name2(matchSpotify, "matchSpotify");
...
__name2(resolveDeezerArtist, "resolveDeezerArtist");
...
__name2(getDeezerAlbums, "getDeezerAlbums");
...
__name2(getDeezerAlbumTracks, "getDeezerAlbumTracks");
...
__name2(searchArtistTracks, "searchArtistTracks");
...
__name2(getAlbumDetail, "getAlbumDetail");
...
__name2(getArtistDetail, "getArtistDetail");
...
function buildSearchQuery({ genres, fromYear, toYear }) {
  ...
  if (fromYear != null || toYear != null) {
    ...
    const to = toYear ?? (/* @__PURE__ */ new Date()).getUTCFullYear() + 1;
...
async function getToken3(env) {
  ...
  try {
    ...
    const r = await fetchRetry(TOKEN_URL3, {
      method: "POST",
      headers: {
    ...
    await env.STATS.put(TOKEN_KEY3, JSON.stringify(stored), {
      expirationTtl: Math.max(60, (data.expires_in || 3600) - 120)
    });
...
function clampYear3(s) {
  ...
  const n = parseInt(s, 10);
  if (!Number.isFinite(n)) return null;
  const max = (/* @__PURE__ */ new Date()).getUTCFullYear() + 1;
  if (n < 1900) return 1900;
  if (n > max) return max;
  return n;
}
...
async function onRequestPost({ request, env }) {
  ...
  if (!ct.includes("application/json")) {
    return json6({ error: "Invalid content type" }, 415);
  }
  let body;
  try {
  ...
  if (honeypot) return json6({ ok: true }, 200);
  if (!ALLOWED_TYPES.includes(type)) {
    return json6({ error: "Invalid type" }, 400);
  }
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return json6({ error: "Valid email required" }, 400);
  }
  if (!message || message.length < 5) {
    return json6({ error: "Message must be at least 5 characters" }, 400);
  }
  if (!turnstileToken) {
    return json6({ error: "Missing verification" }, 403);
  }
  ...
  if (count >= RATE_LIMIT) {
    return json6({ error: "Rate limit exceeded" }, 429);
  }
  const tsOk = await verifyTurnstile(turnstileSecret, turnstileToken, ip);
  if (!tsOk) {
    return json6({ error: "Verification failed" }, 403);
  }
...
`;
  ...
  if (!r.ok) {
    ...
    return json6({ error: "Email send failed", detail }, 502);
  }
...
async function onRequestPost2({ request, env }) {
  ...
  if (!TOOLS.includes(tool)) {
    return json7({ error: "Unknown tool" }, 400);
  }
...
// .wrangler/tmp/bundle-bl3KsH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bl3KsH/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.031166312274151342.js.map
