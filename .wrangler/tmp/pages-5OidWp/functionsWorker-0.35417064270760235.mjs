...
var TOOLS = [
  "string",
  // catch-all for text randomizers (shuffle, weighted, pw, token, emoji)
  "number",
  // catch-all for number tools (rng, dice, lotto, multi, date, pin)
  "wheel",
  // spinning wheel
  "group",
  // group shuffler
...
async function onRequestGet({ env }) {
  ...
  const daysWithoutAccident = Math.max(
    0,
    Math.floor((Date.now() - ACCIDENT_RESET.getTime()) / 864e5)
  );
  return json2({
    total,
    wheelSpins,
...
// ../.wrangler/tmp/pages-5OidWp/functionsRoutes-0.07618803009197972.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-VXkADT/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-VXkADT/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.35417064270760235.mjs.map
